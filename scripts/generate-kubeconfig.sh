...
kubectl get secret github-actions --namespace "${NAMESPACE}" --output go-template='{{ index .data "ca.crt" }}' | base64 --decode > /tmp/kubeconfig-ca.crt
...
KUBECONFIG_TOKEN=$(kubectl get secret github-actions --namespace "${NAMESPACE}" --output go-template='{{ .data.token }}' | base64 --decode)
...
KUBECONFIG_B64="$(cat /tmp/kubeconfig.yml | base64)"
